...
const HoverEditor = ({ waypoint }: { waypoint: BuilderWaypoint }) => {
  ...
  return (
    ...
    >
      ...
      <Field label="Hover" value={""} />
      <TextField
        size="small"
        label="Time (s)"
        value={p.durationSec ?? ""}
        onChange={(e) =>
          updateAction(waypoint.id, hover.id, {
            durationSec: toInt(e.target.value),
          })
        }
        sx={{ width: "5rem" }}
      />
      <TextField
        ...
        onChange={(e) =>
          updateAction(waypoint.id, hover.id, {
            altitudeFt: toInt(e.target.value),
          })
        }
        sx={{ width: "5rem" }}
      />
...
const DetailsEditor = ({ waypoint }: { waypoint: BuilderWaypoint }) => {
  ...
  const [alt, setAlt] = useState(
    waypoint.altitudeFt != null ? String(waypoint.altitudeFt) : "",
  );
      ...
      <TextField
        ...
        onChange={(e) => {
          ...
          if (e.target.value.trim())
            updateWaypoint(waypoint.id, { name: e.target.value });
        }}
    ...
    >
      <Box sx={{ display: "flex", alignItems: "center", gap: 1.5 }}>
        {editing ? (
          ...
          <Box
            sx={{ display: "flex", alignItems: "baseline", gap: 2.5, flex: 1 }}
          >
            <Field
              label="Alt"
              value={`${(waypoint.altitudeFt ?? 0).toLocaleString()} ft`}
            />
            {leg ? (
              <>
        ...
        <Button
          ...
          sx={{
            flexShrink: 0,
            px: 1.25,
            boxShadow: editing ? "none" : `0 0 10px ${colors.accent}66`,
          }}
        >
          {editing ? "Done" : "Edit"}
        </Button>
