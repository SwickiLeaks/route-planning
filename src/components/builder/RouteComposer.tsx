...
const toName = (token: string): string => (/[a-z]/i.test(token) ? token.toUpperCase() : token);
...
const RouteComposer = () => {
  ...
  const commit = (raw: string) => {
    ...
    let last: LatLng | undefined = route.waypoints[route.waypoints.length - 1]?.position;
      ...
      <Box>
        <Autocomplete
          ...
          renderOption={(props, option) => {
            ...
            return (
              <Box component="li" key={key} {...rest} sx={{ display: 'flex', gap: 1.25, alignItems: 'baseline' }}>
                <Box sx={{ fontFamily: MONO, fontSize: 13, fontWeight: 700, color: colors.accent, minWidth: 46 }}>
                  {option.code}
                </Box>
                <Box sx={{ fontSize: 13, color: colors.white }}>{option.name}</Box>
              </Box>
            );
          }}
          renderInput={(params) => (
        ...
        />
        <Typography variant="caption" sx={{ color: FAINT, mt: 0.5, display: 'block' }}>
          Pick a suggested airport, or type any code/name · separate multiple with spaces or commas
        </Typography>
      </Box>
