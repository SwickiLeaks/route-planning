...
export class MissionClient {
  ...
  async startTransaction(
    ...
    console.log("Testing StartTransaction Response: ", res);
  ...
  async addRoute(missionId: string): Promise<string> {
    console.log("Before StartTransaction ID: ", this.transactionId);
    await this.startTransaction("", "Add route");
    console.log("After StartTransaction ID: ", this.transactionId);
    try {
      const res = await this.client.addChild({
        client: this.callId(),
