 ...
 * Service client.
 *
 * All transport concerns live here so hooks and components only ever see the
 * proto-derived types from `@/types/proto`.
 *
 * TODO: currently returns local sample data. When the gRPC services are wired
 * up, replace the body of `fetchRoutes` with the real call — the signature is
 * deliberately already `Promise<Route[]>` so nothing downstream has to change.
 */
import type { Route } from '@/types/proto';

/**
 * Sample Middle Tennessee routes, all within the offline map extract.
 * Altitudes are MSL; field elevations around here run 400–600 ft.
 */
